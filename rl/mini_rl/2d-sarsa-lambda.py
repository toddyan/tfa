...
class Agent:
    ...
    def update_q_table(self, state, action, state_, action_, r, terminated):
        if not self.q_table.has_key(state):
            ...
            self.eligibility[state] = np.zeros(self.actions.shape, dtype=np.float32)
        if not self.q_table.has_key(state_):
            self.q_table[state_] = np.zeros(self.actions.shape, dtype=np.float32)
            self.eligibility[state_] = np.zeros(self.actions.shape, dtype=np.float32)
        ...
        for s in self.eligibility.keys():
            self.q_table[s] += self.learning_rate * self.eligibility[s] * delta
        for s in self.eligibility.keys():
            self.eligibility[s] *= self.impack_decay * self.lambda_
        if terminated:
            self.q_table[state_][:] = r
    def peep(self):
        return 'explore:' + str(self.explore) + '\n' \
               + '\n'.join([str(l[0]) + "\t\t" + str(l[1]) for l in self.q_table.items()]) \
               + '\n\n' + '\n'.join([str(l[0]) + "\t\t" + str(l[1]) for l in self.eligibility.items()]) \
