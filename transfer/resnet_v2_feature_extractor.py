...
class ResNetV2FeatureExtractor:
    def __init__(self, checkpoint_path, sess):
    ...
    def get_feature(self, input_x, sess):
        fea = sess.run(self.features, feed_dict={self.x: input_x})
        return fea


sess = tf.Session()
checkpoint_path = globalconf.get_root() + "transfer/resnet_v2/inception_resnet_v2_2016_08_30.ckpt"
resnet = ResNetV2FeatureExtractor(checkpoint_path,sess)
...
fea = resnet.get_feature(metrix, sess)
